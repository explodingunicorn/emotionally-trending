...
const TwitterClient = new Twitter$1({
...
const Twitter = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	let { trends } = $$props;
	console.log(trends);
	if ($$props.trends === void 0 && $$bindings.trends && trends !== void 0) $$bindings.trends(trends);
...
<div>${each(trends, trend => `<a${add_attribute("href", trend.url, 0)} target="${"_blank"}" class="${"svelte-1jvspx4"}">${escape(trend.name)}</a>`)}</div>`;
});
...
const manifest = {
	server_routes: [
		{
			// twitter/trending.json.js
			pattern: /^\/twitter\/trending\.json$/,
			handlers: route_0,
			params: () => ({})
		},
	...
	pages: [
		...
		{
			// twitter/index.svelte
			pattern: /^\/twitter\/?$/,
			parts: [
				{ name: "twitter", file: "twitter/index.svelte", component: Twitter, preload: preload }
			]
		},
