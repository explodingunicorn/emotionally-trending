// @ts-ignore
import Sentiment from '../sentiment/lib/index';
...
type WordDict = { [key: string]: number };

const countWords = (words: string[], wordDict: WordDict) => {
	words.forEach((word) => {
		if (!wordDict[word]) {
			wordDict[word] = 1;
		} else {
...
const getMostUsedWords = (wordDict: WordDict) => {
	const wordArr: { word: string; count: number }[] = [];
...
export const analyzeTweets = (tweets: any) => {
	let scoreCount = 0,
		comparativeCount = 0;
	...
	const data = tweets.map((tweet: any) => {
		const { score, comparative, positive, negative } = sentiment.analyze(tweet.full_text, options);
